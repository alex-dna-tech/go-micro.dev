...
- Read the [0→hero reference path](zero-to-hero) for the CI-verified
  lifecycle contract.
- Run the [no-secret first-agent transcript](no-secret-first-agent) or [`examples/support`](https://github.com/micro/go-micro/tree/master/examples/support) for the no-secret support-desk lifecycle.
...
- Read [Debugging your agent](debugging-agents) when a chat turn does not call the tool you expected, loops, refuses a call, loses memory, or fails after a flow handoff.
- Read [Agents and Workflows](agents-and-workflows) when you are ready to
