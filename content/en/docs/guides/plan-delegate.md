...
The plan is persisted under `agent/{name}/plan` in the [store](../store) — file-backed by default, Postgres or NATS KV in production — and re-injected into the system prompt on subsequent turns. Memory survives restarts.
...
- [Agent Integration Patterns](agent-patterns) — Pattern 9 covers planning and delegation
- [AI Integration](../ai-integration) — agents, flows, and the model interface
- [Store](../store) — where agent memory lives
