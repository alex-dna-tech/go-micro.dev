...
title: "Payments (x402)"
weight: 140
description: "Go Micro can require a payment before a tool runs, using x402 — the open HTTP 402 Payment Required standard for stablecoin payments, designed for AI agents and onchain APIs. It lets every Go Micro end"
---
...
- **A paying agent needs a budget.** On the agent side, an unattended agent that spends money needs a spend cap next to `MaxSteps` and `ApproveTool` — see [Plan & Delegate](plan-delegate) for the guardrail model. This is active work.

## See also

- [Building Effective Agents — Agents and Workflows](agents-and-workflows)
- [MCP & AI Agents](../mcp)
