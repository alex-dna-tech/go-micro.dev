...
loop with [agent guardrails](agent-guardrails) (e.g. token/spend limits)
and [paid tools](x402-payments) (per-call metering) so a background loop
...
(before and after the step) through its [Checkpoint](../deployment), and a
...
- [Agents and Workflows](agents-and-workflows) — flows vs. agents
- [Agent Guardrails](agent-guardrails) — bounding what a loop can do
- [Plan & Delegate](plan-delegate) — splitting work across agents
