...
The simplest approach. Deploy compiled binaries to a Linux server and manage them with systemd. This is the model described in the [Deployment Guide](deployment).
...
See [Observability](observability) for details on logs, metrics, and traces.
...
- [Deployment](deployment) — Deploy services to a Linux server with systemd
- [Registry](registry) — Service discovery backends
- [Architecture](architecture) — Go Micro design and components
- [Observability](observability) — Logs, metrics, and tracing
- [Performance](performance) — Performance characteristics and tuning
